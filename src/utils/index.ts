...
const fetchPredictionStatus = async (url: string, setError: (error: string) => void): Promise<Prediction | null> => {
    try {
        ...
        await sleep(2500); // Adjust the delay as needed
        return fetchPredictionStatus(url, setError);
    } catch (error) {
...
export const generateTheme = async (
    ...
    return fetchPredictionStatus('/api/generate-theme/' + prediction.id, setError);
};
...
export const generateOrchestralBrief = async (
    ...
    return fetchPredictionStatus('/api/generate-brief/' + prediction.id, setError);
};
...
export const generateMusic = async (
    ...
    return fetchPredictionStatus('/api/generate-music/' + prediction.id, setError);
};